...
public class RedisRateLimitingDAO extends BaseDynoDAO implements RateLimitingDAO {
     ...
     * Task#getRateLimitPerFrequency()} and {@link Task#getRateLimitFrequencyInSeconds()} if not checks the {@link Task}
     * is rate limited or not based on {@link Task#getRateLimitPerFrequency()} and {@link
     * Task#getRateLimitFrequencyInSeconds()}
     * <p>
     * The rate limiting is implemented using the Redis constructs of sorted set and TTL of each element in the rate
     * limited bucket.
     * <ul>
     *     <li>All the entries that are in the not in the frequency bucket are cleaned up by leveraging {@link JedisProxy#zremrangeByScore(String, String, String)},
     ...
     *     this is achieved by using {@link JedisProxy#zcount(String, double, double)} </li>
     ...
     * @return true: If the {@link Task} is rateLimited false: If the {@link Task} is not rateLimited
     */
    @Override
    public boolean exceedsRateLimitPerFrequency(Task task, TaskDef taskDef) {
        ...
        ImmutablePair<Integer, Integer> rateLimitPair = Optional.ofNullable(taskDef)
            .map(definition -> new ImmutablePair<>(definition.getRateLimitPerFrequency(),
